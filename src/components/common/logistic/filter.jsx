...
const FilterNama = ({ handleSelectChange }) => {
  ...
  const [filteredName, setFilteredName] = useState('Mekkah'); // Set initial value

  useEffect(() => {
    axios
      .get('http://localhost:8080/api/data/logistic/transport')
      .then((response) => {
        const distinctNames = new Set(
          response.data.data.map((item) => item.kota_logistic)
        );
